# YCM extra conf intended to be used globally. FlagsForFile will attempt to
# figure out flags in two different ways in the following order:
#
#     * from a clang compilation database found in ['compile_commands.json',
#     'build/compile_commands.json', '_build/compile_commands.json'] somewhere
#     along the parent path of the file being edited.
#
#     * from a .clang_complete file found somewhere along the path of the file
#     being edited.
#
# If both method fail then we assume we start with an empty list of flags. In
# the end, regardless of success or failure of the above methods, we'll append
# our base flags.
import os
import re
import subprocess
...
# We won't get suggestions for standard C++ includes unless we explicitly add
# them to our list of flags.
RE_CLANG_CAPTURE_INCLUDES = \
    re.compile(r'.*?^#include <\.\.\.> search starts here:\s'
               r'(?P<paths>.*\s)'
               r'End of search list.$', re.MULTILINE | re.DOTALL)
...
def GetStandardIncludes():
    ...
    if call_result.returncode == 0:
        ...
        if match:
            ...
            return [''.join(['-isystem', os.path.realpath(path)])
        ...
        for path_flag in PATH_FLAGS:
            if flag.startswith(path_flag):
                ...
                if not os.path.isabs(path):
                    new_flags.append(''.join([path_flag,
...
def FindFileUpwards(file_name, start_path, additional_folders=[]):
    candidates = [file_name] + [os.path.join(folder, file_name)
                                for folder in additional_folders]
    current_root = os.path.realpath(start_path)
    while current_root != '/':
        candidate_paths = [os.path.join(current_root, candidate)
        ...
        for extension in SOURCE_EXTENSIONS:
            ...
            for replacement_file in replacement_files:
                if os.path.exists(replacement_file):
                    compilation_info = \
                        compilation_database.GetCompilationInfoForFile(
                            replacement_file)
...
def FlagsFromCompilationDatabase(root, filename):
    try:
        compilation_db_path = FindFileUpwards('compile_commands.json', root,
                                              BUILD_DIRECTORIES)
...
def FlagsFromClangCompleteFile(root):
    try:
        ...
        return MakeRelativePathsInFlagsAbsolute(
            flags, os.path.dirname(clang_complete_file))
    except:
        return []
